...
#define EPITECH_SERIAL_MAJOR 240 /* experimental range */
#define SERIAL_MODULE_NAME "EpitechSerial"

#define DRIVER_AUTHOR "Nicolas KLARMAN <nickoas@gmail.com>"
#define DRIVER_DESC "Epitech Serial Driver"
...
char modname[] = SERIAL_MODULE_NAME;
...
{
  printk("<EPITECH_SERIAL> Installing \"%s\" module\n", modname);
  ...
  printk("<EPITECH_SERIAL> Interrupt-ID: %02X \n", inb(UART_INTR_ID));
 
  create_proc_read_entry(modname, 0, NULL, my_proc_read, NULL);
  return register_chrdev(EPITECH_SERIAL_MAJOR, modname, &my_fops);
}

static void __exit uart_exit(void)
{
  unregister_chrdev(EPITECH_SERIAL_MAJOR, modname);
  ...
  printk("<EPITECH_SERIAL> Removing \'%s\' module\n", modname);
}

/* Module declaration init and exit */
...
ssize_t my_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
  ...
  if ((line_status & 1) == 0)
	...
	return 0;
      ...
      if (wait_event_interruptible(wq_recv, (inb(UART_LINE_STAT) & 1)))
	return -EINTR;
    }
...
ssize_t my_write(struct file *file, const char *buf, size_t len, loff_t *pos)
{
  ...
  if ((modem_status & 0x10) != 0x10)
	...
	return 0;
      ...
      if (wait_event_interruptible(wq_xmit, (inb(UART_MODEM_STAT) & 0x10) == 0x10))
	return -EINTR; 
    }
